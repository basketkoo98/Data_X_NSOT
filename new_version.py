...
nba = pd.read_csv('finalcsv_updated.csv')


#Loading in the predicted csv
pred = pd.read_csv("pred_updated.csv")
...
def pCap_chart(selected_player_display):
    ...
    ax = fig1.subplots()
    print(filterNameNBA)
    pCapfiltered = filterNameNBA[["Season", "pCap"]]   #['pCap'].loc[::-1]
    pCapfiltered = pCapfiltered.set_index("Season")
    pCapfiltered2 = filterNameNBA[["Season", "Predicted pCap"]]  #['pCap'].loc[::-1]
...
def graph_2000to2020(selected_player_display, selected_column, tuple, filter1, selected_filter):
    ...
    if filter1:
        ...
        fgp = fgp[(fgp[selected_filter] >= tuple[0]) & (fgp[selected_filter] <= tuple[1])]
...
def graph_2021(selected_player_display, selected_column2, tuple, filter2, selected_filter):
    ...
    copy = pred
    if filter2:
        copy = pred[(pred[selected_filter] >= tuple[0]) & (pred[selected_filter] <= tuple[1])]

    fgp = copy[[selected_column2,'pCap']]
    fgp2 = copy.loc[selected_for_pred][[selected_column2, 'pCap']]

    cluster_number = pred.loc[selected_for_pred]["sort.km.res.cluster."] 
    pred_cluster = copy[copy["sort.km.res.cluster."] == cluster_number]
...
#Adding Player headshot
player_Headshot = get_player_headshot(selected_player_display)
col1, mid, col2, mid2, col3 = st.beta_columns([4, 0.5, 5, 1, 20])
col1.image(player_Headshot, use_column_width=True)
...
# if player from one csv appears in other csv:
#     display the 
